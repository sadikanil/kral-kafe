{{-- Siradaki deneme hatirlaticisi. Beklenen: $upcomingExams (yakindan uzaga),
     ...
     Deneme adi yalnizca deneme kulubunde (Dalga 19). --}}
...
@if($upcomingExams->isNotEmpty())
    ...
    <div class="alert {{ $yakin ? 'alert-warning' : 'alert-info' }} animate-slide-up mb-3">
        ...
        {{ $detay ? $ilk->title . ' (' . $ilk->exam_type->label() . ')' : $ilk->exam_type->label() }} —
        ...
        @if($upcomingExams->count() > 1)
            <br><small class="text-muted">
                ...
                @foreach($upcomingExams->slice(1) as $deneme)
                    {{ $detay ? $deneme->title : $deneme->exam_type->label() }} ({{ $deneme->exam_date->format('d.m') }}){{ $loop->last ? '' : ',' }}
                @endforeach
            </small>
        @endif
