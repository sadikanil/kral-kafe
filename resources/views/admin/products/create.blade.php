            ...
            <form method="POST" action="{{ route('admin.products.store') }}" enctype="multipart/form-data">
                    ...
                    <div class="form-group" style="width: 120px;">
                        ...
                        <select id="emoji" name="emoji" class="form-control @error('emoji') is-invalid @enderror">
                            ...
                            <optgroup label="İçecekler">
                                <option value="☕" {{ old('emoji') == '☕' ? 'selected' : '' }}>☕ Kahve</option>
                                <option value="🍵" {{ old('emoji') == '🍵' ? 'selected' : '' }}>🍵 Çay</option>
                                <option value="🥤" {{ old('emoji') == '🥤' ? 'selected' : '' }}>🥤 İçecek</option>
                                <option value="🧃" {{ old('emoji') == '🧃' ? 'selected' : '' }}>🧃 Meyve Suyu</option>
                                <option value="🥛" {{ old('emoji') == '🥛' ? 'selected' : '' }}>🥛 Süt</option>
                            </optgroup>
                            <optgroup label="Yiyecekler">
                                <option value="🥯" {{ old('emoji') == '🥯' ? 'selected' : '' }}>🥯 Simit/Poğaça</option>
                                <option value="🥪" {{ old('emoji') == '🥪' ? 'selected' : '' }}>🥪 Sandviç</option>
                                <option value="🥐" {{ old('emoji') == '🥐' ? 'selected' : '' }}>🥐 Kruvasan</option>
                                <option value="🍪" {{ old('emoji') == '🍪' ? 'selected' : '' }}>🍪 Kurabiye</option>
                                <option value="🍫" {{ old('emoji') == '🍫' ? 'selected' : '' }}>🍫 Çikolata</option>
                                <option value="🍬" {{ old('emoji') == '🍬' ? 'selected' : '' }}>🍬 Şeker</option>
                                <option value="🍟" {{ old('emoji') == '🍟' ? 'selected' : '' }}>🍟 Cips</option>
                            </optgroup>
                        </select>
                        @error('emoji')
                            <span class="invalid-feedback">{{ $message }}</span>
                ...
                <div class="form-group">
                    ...
                    <input type="text" id="category" name="category" list="categoryList"
                        ...
                        placeholder="Örn: İçecek, Atıştırmalık" autocomplete="off">
                ...
                <div class="d-flex gap-2">
                    ...
                    <div class="form-group" style="flex: 1;">
                        ...
                        <input type="text" id="unit_type" name="unit_type" list="unitList"
                            class="form-control @error('unit_type') is-invalid @enderror" value="{{ old('unit_type') }}"
                            placeholder="Seçin veya yazın" required autocomplete="off">
