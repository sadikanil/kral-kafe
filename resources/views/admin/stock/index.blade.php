...
@section('title', 'Stok Yönetimi - Kral Kafe')
@section('page-title', 'Stok Sayımı')

@section('content')
    <div class="alert alert-info mb-4">
        💡 Stok sayımı yapmak için bir lokasyon seçin, ürün fotoğraflarını çekin ve AI ile analiz ettirin.
    </div>
    
    <!-- Lokasyon Seçimi -->
    <div class="card mb-4">
        <div class="card-header">
            <h4>Lokasyon Seçin</h4>
        </div>
        <div class="card-body">
                ...
                @forelse($locations as $location)
                    <a href="{{ route('admin.stock.capture', $location) }}" class="card" style="text-decoration: none;">
                        <div class="card-body text-center">
                            ...
                            <span class="text-muted">{{ $location->products->count() }} ürün</span>
                        </div>
                    </a>
                @empty
                    <p class="text-muted">Henüz lokasyon eklenmemiş.</p>
                @endforelse
            </div>
        </div>
    </div>
